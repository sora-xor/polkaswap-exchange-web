...
describe('vault events query', () => {
  ...
  it('fetches SubQuery vault events and parses amount and timestamp values', async () => {
  ...
  it('fetches Subsquid vault events from nodes with the same pagination variables', async () => {
    indexerMocks.fetchEntities.mockResolvedValue({
      totalCount: 1,
      nodes: [createVaultEvent('3', '3.75', 'Withdraw', 1_700_000_003)],
    });
    indexerMocks.currentIndexer = createIndexer(IndexerType.SUBSQUID);

    const result = await fetchVaultEvents({ id: 'vault-1', first: 10, offset: 0 });
    ...
    expect(result.items[0]?.amount?.toString()).toBe('3.75');
    expect(result.items[0]?.timestamp).toBe(1_700_000_003_000);
  });
  ...
  it('returns an empty response when Subsquid returns no vault event data', async () => {
    indexerMocks.fetchEntities.mockResolvedValue(null);
    indexerMocks.currentIndexer = createIndexer(IndexerType.SUBSQUID);
  ...
  it('returns an empty response for unsupported indexer types without making requests', async () => {
