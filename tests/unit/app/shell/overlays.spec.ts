...
vi.mock('@/components/shared/Dialog/SelectSoraAccount.vue', () => ({
  default: defineComponent({ name: 'SelectSoraAccountDialogStub', template: '<div class="select-sora-account-dialog-stub" />' }),
}));
...
describe('AppShellOverlays', () => {
  ...
  it('wires dark-page and local-storage actions through the shell context and renders notification copy', async () => {
    ...
    await wrapper.findComponent({ name: 'AppBrowserNotifsLocalStorageOverrideStub' }).vm.$emit('delete-data-local-storage');
