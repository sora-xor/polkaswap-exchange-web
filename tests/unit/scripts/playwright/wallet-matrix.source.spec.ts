...
const walletMatrixSource = fs.readFileSync(
  path.resolve(process.cwd(), 'scripts/playwright/wallet-matrix.mjs'),
  'utf8'
);
...
describe('wallet matrix smoke runner source', () => {
  ...
  it('uses isolated runtime copies of wallet extension profiles by default', () => {
    expect(walletMatrixSource).toContain("const USE_RUNTIME_PROFILE_COPIES = process.env.WALLET_MATRIX_RUNTIME_COPIES !== '0';");
  ...
  it('prefers the installed Chrome channel for persistent extension automation', () => {
    expect(walletMatrixSource).toContain("const PERSISTENT_CONTEXT_CHANNEL = process.env.WALLET_MATRIX_CHANNEL || 'chrome';");
