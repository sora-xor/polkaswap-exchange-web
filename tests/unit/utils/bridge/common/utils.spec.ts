...
describe('bridge common utils', () => {
  ...
  it('waitForEvmTransactionMined follows replacement transactions and reports the new hash', async () => {
    ...
    isEthersErrorMock.mockImplementation((error: unknown, code: string) => code === 'TRANSACTION_REPLACED' && error === replacedError);
  ...
  it('onEvmTransactionPending rejects transactions without an EVM hash', async () => {
    ...
    ).rejects.toThrow('[onEvmTransactionPending] Evm transaction hash is empty');
  });
  ...
  it('onEvmTransactionPending updates mined receipt metadata', async () => {
    ...
    await onEvmTransactionPending(
      'tx-id',
      () => ({ externalHash: '0xevm' }) as any,
      updateTransaction
    );
  ...
  it('onEvmTransactionPending stores replacement transaction hashes before mined metadata', async () => {
    ...
    await onEvmTransactionPending(
      'tx-id',
      () => ({ externalHash: '0xoriginal' }) as any,
      updateTransaction
    );
  ...
  it('onEvmTransactionPending resets stale hashes when no mined receipt is available', async () => {
    ...
    await expect(
      onEvmTransactionPending(
        'tx-id',
        () => ({ externalHash: '0xmissing' }) as any,
        updateTransaction
      )
    ).rejects.toThrow("[onEvmTransactionPending]: Ethereum transaction not found, hash: 0xmissing. 'externalHash' is reset");
