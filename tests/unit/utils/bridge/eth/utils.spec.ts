...
describe('ETH bridge utils', () => {
  ...
  it('waits for user action only after an EVM rejection with an unsigned destination part', () => {
    ...
    expect(isWaitingForAction({ type: 'EthBridgeIncoming', transactionState: ETH_BRIDGE_STATES.EVM_REJECTED } as any))
      .toBe(false);
    expect(isWaitingForAction({ type: 'EthBridgeOutgoing', transactionState: ETH_BRIDGE_STATES.EVM_PENDING } as any))
      .toBe(false);
  });
  ...
  it('validates required fields before waiting for incoming requests', async () => {
    ...
    await expect(waitForIncomingRequest({ externalHash: '0xexternal', externalNetwork: Number.NaN } as any)).rejects
      .toThrow('[Bridge]: Tx externalNetwork should be a number, NaN received');
  });
