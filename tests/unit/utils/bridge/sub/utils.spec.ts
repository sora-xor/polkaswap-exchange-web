...
describe('Substrate bridge utils', () => {
  ...
  it('extracts accepted and dispatched message nonces', () => {
    expect(
      getMessageAcceptedNonces([event('substrateBridgeOutboundChannel', 'MessageAccepted', [codec(0), codec(7), codec(9)])])
    ).toEqual([7, 9]);
  ...
  it('matches xcmApp AssetAddedToChannel transfer payloads', () => {
    ...
    expect(isAssetAddedToChannel(event('system', 'ExtrinsicSuccess'), {} as any, 'recipient', '100' as any, chainApi as any))
      .toBe(false);
    expect(
