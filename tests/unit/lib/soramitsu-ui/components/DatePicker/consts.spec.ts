...
describe('DatePicker consts', () => {
  ...
  it('exports calendar labels and half-hour time points', async () => {
    const { daysNames, months, monthsShort, TIME_POINTS } =
      await import('@/lib/soramitsu-ui/components/DatePicker/consts');
  ...
  it('exports custom option metadata and default shortcut groups', async () => {
    const { CUSTOM_OPTION, CUSTOM_OPTION_VALUE, DEFAULT_SHORTCUTS } =
      await import('@/lib/soramitsu-ui/components/DatePicker/consts');
