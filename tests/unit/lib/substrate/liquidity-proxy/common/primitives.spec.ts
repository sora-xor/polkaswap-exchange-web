...
describe('liquidity proxy primitives', () => {
  ...
  it('rescales chunks by sides, ratios, and arithmetic operations', () => {
    ...
    expect(asNumber(sample.rescaleBySideAmount(new SideAmount(fp(3), SwapVariant.WithDesiredInput)).output)).toBeCloseTo(
      6
    );
    expect(asNumber(sample.rescaleBySideAmount(new SideAmount(fp(6), SwapVariant.WithDesiredOutput)).input)).toBeCloseTo(
      3
    );
  ...
  it('calculates precision steps for input and output amount precision', () => {
    ...
    expect(new SwapLimits(null, null, null).getPrecisionStep(sample, SwapVariant.WithDesiredInput).toString()).toBe('0');
  });
