...
describe('pool XYK utils', () => {
  ...
  it('rejects identical assets and pairs without the configured base asset', () => {
    ...
    expect(() => getPairInfo('base', 'asset-a', 'asset-b')).toThrow(Errors.BaseAssetIsNotMatchedWithAnyAssetArguments);
  });

  it('resolves allowed chameleon pools through the runtime registry', () => {
    getChameleonPoolsMock.mockReturnValue(['kxor', ['eth', 'dai']]);

    expect(getPairInfo('xor', 'kxor', 'eth')).toEqual([{ baseAssetId: 'xor', targetAssetId: 'eth' }, 'kxor', true]);
    expect(getPairInfo('xor', 'dai', 'kxor')).toEqual([{ baseAssetId: 'xor', targetAssetId: 'dai' }, 'kxor', true]);
  });
  ...
  it('rejects restricted or unrelated chameleon pairs', () => {
    ...
    expect(() => getPairInfo('xor', 'asset-a', 'asset-b')).toThrow(Errors.BaseAssetIsNotMatchedWithAnyAssetArguments);
  });
});
