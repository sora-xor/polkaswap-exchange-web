...
describe('liquidity proxy XYK pool pallet', () => {
  ...
  it('calculates no-impact XYK prices for both swap variants', () => {
    expect(asNumber(quoteWithoutImpact(BASE, SYNTHETIC_BASE, BASE, TARGET, fp(10), true, createPayload(), true))).toBeCloseTo(
      4.97
    );
    expect(asNumber(quoteWithoutImpact(BASE, SYNTHETIC_BASE, TARGET, BASE, fp(20), false, createPayload(), true))).toBeCloseTo(
      10.06036217303823
    );
  });

  it('calculates no-impact prices for target desired-input and base desired-output variants', () => {
    const targetDesiredInput = quoteWithoutImpact(BASE, SYNTHETIC_BASE, TARGET, BASE, fp(10), true, createPayload(), true);
    const baseDesiredOutput = quoteWithoutImpact(BASE, SYNTHETIC_BASE, BASE, TARGET, fp(5), false, createPayload(), true);
