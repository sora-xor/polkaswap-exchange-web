...
describe('liquidity proxy XST pallet', () => {
  ...
  it('returns safe zero quotes for invalid paths, zero amounts, exceeded limits, and invalid fees', () => {
    ...
    expect(
      quote(BASE, XST, XSTUSD, XST, fp(1), true, createPayload({ feeRatio: fp('0.99'), dynamicFee: '0.02' }), true)
        .amount
        .toString()
    ).toBe('0');
  });
  ...
  it('returns empty step quotes for zero input or zero configured limit', () => {
    ...
    expect(
      stepQuote(BASE, XST, XST, XSTUSD, fp(1), true, createPayload({ limit: 0 }), true, 4).chunks
    ).toEqual([]);
  });
...
const createPayload = ({ dynamicFee = '0.01', feeRatio = fp('0.02'), floorPrice = 1, limit = 100 }: PayloadOptions = {}) =>
  ({
    enabledAssets: {
      tbc: [],
      xst: {
