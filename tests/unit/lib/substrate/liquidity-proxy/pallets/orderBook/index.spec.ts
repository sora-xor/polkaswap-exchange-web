...
describe('liquidity proxy order book pallet', () => {
  ...
  it('calculates no-impact order-book prices from the best ask and best bid', () => {
    expect(
      asNumber(quoteWithoutImpact(BASE, SYNTHETIC_BASE, BASE, ASSET, fp(20), true, createPayload(), false))
    ).toBeCloseTo(10);
    expect(
      asNumber(quoteWithoutImpact(BASE, SYNTHETIC_BASE, ASSET, BASE, fp(9), false, createPayload(), false))
    ).toBeCloseTo(5);
  });

  it('calculates no-impact prices for the other buy/sell variants', () => {
    const buyDesiredOutput = quoteWithoutImpact(
      BASE,
      SYNTHETIC_BASE,
      BASE,
      ASSET,
  ...
  it('throws for step quotes when the order book is not exchangeable or missing', () => {
    ...
    expect(() =>
      stepQuote(BASE, SYNTHETIC_BASE, BASE, 'missing-book', fp(1), true, createPayload(), false, 10)
    ).toThrow(Errors.CantExchange);
  });
