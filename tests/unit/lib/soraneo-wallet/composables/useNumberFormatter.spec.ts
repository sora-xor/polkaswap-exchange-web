...
import { useNumberFormatter } from '@/lib/soraneo-wallet/src/composables/useNumberFormatter';

describe('wallet lib useNumberFormatter', () => {
  ...
  it('creates and formats numbers from natural and codec values', () => {
    ...
    expect(formatter.getFPNumberFromCodec(codecValue, 2).toString()).toBe(
      FPNumber.fromCodecValue(codecValue, 2).toString()
    );
