import { H as useAssetsStore, h as computed, W as router, V as PageNames, F as FPNumber, X as XOR } from "./index-73GArslZ.js";
import { u as useBridgeStore } from "./index-FPtsBGoq.js";
function useBridgeCore() {
  const assetsStore = useAssetsStore();
  const bridgeStore = useBridgeStore();
  const isSoraToEvm = computed(() => bridgeStore.isSoraToEvm);
  const asset = computed(() => bridgeStore.asset);
  ...
  const nativeToken = computed(() => bridgeStore.nativeToken);
  ...
  const outgoingMaxAmount = computed(() => {
    const limits = [];
  ...
  const outgoingMinAmount = computed(() => {
    if (!outgoingMinLimit.value) return null;
    return outgoingMinLimit.value.add(externalTransferFeeFP.value);
  });
  const incomingMaxAmount = computed(() => {
    if (isSidechainAsset.value) return null;
    return assetLockedBalance.value ?? null;
  });
  const getTransferMaxAmount = (isOutgoing) => isOutgoing ? outgoingMaxAmount.value : incomingMaxAmount.value;
  const getTransferMinAmount = (isOutgoing) => isOutgoing ? outgoingMinAmount.value : incomingMinLimit.value;
  const isGreaterThanTransferMaxAmount = (amount, currentAsset, isOutgoing, isRegisteredAsset = true) => {
  ...
  const isLowerThanTransferMinAmount = (amount, currentAsset, isOutgoing, isRegisteredAsset = true) => {
