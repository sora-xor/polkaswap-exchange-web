...
export function useSwapRouteSync(withApi: WithApi) {
  ...
  const syncSwapRoutePair = async (firstAddress = '', secondAddress = ''): Promise<void> => {
    ...
    const hasResolvedRoutePair =
      tokenFrom.value?.address === normalizedPair.firstAddress &&
      tokenTo.value?.address === normalizedPair.secondAddress;
  ...
  const routeTokenTo = computed(() =>
    secondRouteAddress.value
      ? (assetsStore.assetDataByAddress(secondRouteAddress.value) as Nullable<AccountAsset>)
      : null
  );
