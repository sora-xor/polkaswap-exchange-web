...
const actions = {
  ...
  async updateRegisteredAssets ({ commit, dispatch }) {
    try {
      ...
      const enabledRegisteredAssets = registeredAssets.filter(item => !DISABLED_ASSETS.includes(item.address))
      const preparedRegisteredAssets = await Promise.all(enabledRegisteredAssets.map(async item => {
