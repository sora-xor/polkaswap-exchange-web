...
export default class TransactionMixin extends Mixins(mixins.NumberFormatterMixin, TranslationMixin, LoadingMixin) {
  ...
  private getMessage(value?: History): string {
    ...
    if (
      [Operation.AddLiquidity, Operation.CreatePair, Operation.RemoveLiquidity, Operation.Swap].includes(value.type)
    ) {
  ...
  handleChangeTransaction(value: History): void {
    if (
      !value ||
      !value.status ||
      ![TransactionStatus.Finalized, TransactionStatus.Error].includes(value.status as any)
    ) {
      return;
    }
