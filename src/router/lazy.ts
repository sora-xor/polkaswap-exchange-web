...
/**
 * Retries transient async-import failures for route-level dynamic imports.
 * This is especially useful on public gateways (IPFS/CDN) where bursts can
 * return temporary 429/aborted responses for chunk files.
 */
...
export const lazyView = (name: string) => () => loadAsyncImportWithRetry(() => import(`@/views/${name}.vue`));
