  ...
  methods: {
    getTitle(this: any, value?: History): string {
      ...
      if (value.type === Operation.ReferralSetInvitedUser) {
        const isInvitedUser = (this.account as PolkadotJsAccount).address === value.from;
    ...
    getOperationMessage(this: any, value?: History, hideAmountValues = false): string {
      if (!value || !Object.values(Operation).includes(value.type as Operation)) {
        return '';
      }
      const params = { ...value } as any;
      if (accountIdBasedOperations.includes(value.type)) {
        const isRecipient = (this.account as PolkadotJsAccount).address === value.to;
