...
const RAW_BASE_URL = process.env.SWAP_SMOKE_BASE_URL || 'http://127.0.0.1:8896';
const BASE_URL = resolveAppBaseUrl(RAW_BASE_URL, process.env.SWAP_SMOKE_PREFIX);
const SWAP_URL = resolveRouteUrl(RAW_BASE_URL, '#/swap', process.env.SWAP_SMOKE_PREFIX);
  ...
  try {
    ...
    page.on('console', (message) => {
      if (message.type() === 'error') {
        ...
        if (/ERR_CERT_COMMON_NAME_INVALID|ERR_NAME_NOT_RESOLVED/i.test(text)) return;
        errors.push(text);
      }
    });
    ...
    const selectTokenByRow = async (buttonIndex, rowIndex) => {
      ...
      while (Date.now() < deadline && rowCount <= rowIndex) {
        await sleep(200);
