...
if (files.length === 0) {
  console.error('Usage: lazyWalletCore.mjs [--source <module>] [--loader <path>] <files...>');
  process.exit(1);
}
...
for (const file of files) {
  ...
  if (runtimeSpecifiers.length === 0) {
    if (touched) {
      ...
      output = output.replace(/';(?=(import|const|let|var))/g, "';\n").replace(/";(?=(import|const|let|var))/g, '";\n');
    ...
    if (existingDecl) {
      ...
      if (declarator && t.isObjectPattern(declarator.id)) {
        for (const prop of objectProperties) {
          const keyName = t.isRestElement(prop) ? prop.argument.name : t.isIdentifier(prop.key) ? prop.key.name : null;
