...
async function collect(url) {
  ...
  const settingsTrigger = page.locator(
    '.app-header-menu .header-menu__button i[class*="s-icon-grid-block-align-left-24"], .app-header-menu i.s-icon-grid-block-align-left-24, .app-header-menu .header-menu__button'
  );
  ...
  const data = await page.evaluate(() => {
    ...
    const sidebarRows = Array.from(document.querySelectorAll('.app-sidebar .menu-item, .app-sidebar .el-menu-item'))
      .map((item) => {
        const textEl = item.querySelector('.title, p, span, .sidebar-item-content__title');
        const text = (textEl?.textContent || item.textContent || '').replace(/\s+/g, ' ').trim();
        const iconEl = item.querySelector('i[class*="s-icon-"]');
        return { text, icon: iconClass(iconEl) };
      })
      .filter((r) => r.text && r.text.length < 40);

    const headerRows = Array.from(
      document.querySelectorAll('.header-menu .header-menu__item, .header-menu .el-dropdown-menu__item')
    )
      .map((item) => {
    ...
    const settingsTrigger = document.querySelector('.app-header-menu i[class*="s-icon-"]');

    return {
      settingsTrigger: iconClass(settingsTrigger),
      headerControls,
      sidebarRows,
      headerRows,
    };
