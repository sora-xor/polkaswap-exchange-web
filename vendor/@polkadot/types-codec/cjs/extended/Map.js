...
function decodeMapFromU8a(registry, KeyClass, ValClass, u8a) {
  ...
  const [values, decodedLength] = (0, _utils.decodeU8a)(registry, new Array(types.length), u8a.subarray(offset), [
    types,
    [],
  ]);
...
function decodeMapFromMap(registry, KeyClass, ValClass, value) {
  ...
  for (const [key, val] of value.entries()) {
    const isComplex =
      KeyClass.prototype instanceof _Array.AbstractArray ||
      KeyClass.prototype instanceof _Struct.Struct ||
      KeyClass.prototype instanceof _Enum.Enum;
    try {
      output.set(
