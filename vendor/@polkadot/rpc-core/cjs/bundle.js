...
function logErrorMessage(method, { noErrorLog, params, type }, error) {
  ...
  l.error(
    `${method}(${params.map(({ isOptional, name, type }) => `${name}${isOptional ? '?' : ''}: ${type}`).join(', ')}): ${type}:: ${error.message}`
  );
}
...
class RpcCore {
    ...
    for (let s = 0, scount = this.sections.length; s < scount; s++) {
      ...
      for (let m = 0, mcount = methods.length; m < mcount; m++) {
        ...
        if (!this.mapping.has(jsonrpc)) {
          ...
          (0, util_1.lazyMethod)(this[section], method, () =>
            isSubscription
  ...
  _createMethodSend(section, method, def) {
    ...
    const callWithRegistry = async (isScale, values) => {
      ...
      const blockHash =
        blockId && def.params[hashIndex].type === 'BlockNumber' ? await this.#getBlockHash?.(blockId) : blockId;
      const { registry } =
        ...
        return new rxjs_1.Observable((observer) => {
          ...
          try {
            ...
            subscriptionPromise = this._createSubscriber(
              { paramsJson: params.map((p) => p.toJSON()), subName, subType, update },
              errorHandler
            );
          } catch (error) {
            errorHandler(error);
          ...
          return () => {
            ...
            subscriptionPromise
              .then((subscriptionId) =>
                (0, util_1.isNull)(subscriptionId)
  ...
  _formatParams(registry, blockHash, def, inputs) {
    ...
    if (count < reqCount || count > def.params.length) {
      throw new Error(
        `Expected ${def.params.length} parameters${reqCount === def.params.length ? '' : ` (${def.params.length - reqCount} optional)`}, ${count} found instead`
      );
    }
  ...
  _formatStorageData(registry, blockHash, key, value) {
    ...
    const input = isEmpty ? null : isTreatAsHex(key) ? value : (0, util_1.u8aToU8a)(value);
  ...
  _formatStorageSetEntry(registry, blockHash, key, changes, withCache, entryIndex) {
    ...
    const isEmpty = (0, util_1.isNull)(value);
    const input = isEmpty || isTreatAsHex(key) ? value : (0, util_1.u8aToU8a)(value);
