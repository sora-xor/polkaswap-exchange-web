...
class WsProvider {
   ...
   */
  constructor(
    endpoint = defaults_js_1.default.WS_URL,
    autoConnectMs = RETRY_DELAY,
    headers = {},
    timeout,
    cacheCapacity,
    cacheTtl
  ...
  async connect() {
    ...
    try {
      ...
      this.#websocket =
        typeof x_global_1.xglobal.WebSocket !== 'undefined' &&
        (0, util_1.isChildClass)(x_global_1.xglobal.WebSocket, x_ws_1.WebSocket)
  ...
  async unsubscribe(type, method, id) {
    ...
    try {
      return this.isConnected && !(0, util_1.isNull)(this.#websocket) ? this.send(method, [id]) : true;
    } catch {
      return false;
    }
  }
  ...
  #onSocketClose = (event) => {
    const error = new Error(
      `disconnected from ${this.endpoint}: ${event.code}:: ${event.reason || (0, errors_js_1.getWSErrorString)(event.code)}`
    );
    if (this.#autoConnectMs > 0) {
