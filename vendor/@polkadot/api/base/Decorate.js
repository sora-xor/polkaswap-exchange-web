...
export class Decorate extends Events {
  ...
  _decorateCalls({ registry, runtimeVersion: { apis, specName, specVersion } }, decorateMethod, blockHash) {
    ...
    const lazySection = (section) =>
      lazyMethods({}, Object.keys(named[section]), (method) =>
  ...
  _decorateCall(registry, def, stateCall, decorateMethod) {
    ...
    const decorated = decorateMethod((...args) => {
      ...
      const bytes = registry.createType(
        'Raw',
        u8aConcatStrict(args.map((a, i) => registry.createTypeUnsafe(def.params[i].type, [a]).toU8a()))
      );
  ...
  _decorateExtrinsics({ tx }, decorateMethod) {
    ...
    const lazySection = (section) =>
      lazyMethods({}, Object.keys(tx[section]), (method) =>
  ...
  _decorateStorage({ query, registry }, decorateMethod, blockHash) {
    const result = {};
    const lazySection = (section) =>
      lazyMethods({}, Object.keys(query[section]), (method) =>
        blockHash
  ...
  _retrieveMulti(keys, blockHash) {
    ...
    return combineLatest(arrayChunk(keys, PAGE_SIZE_V).map((k) => (blockHash ? query(k, blockHash) : query(k)))).pipe(
      map(arrayFlatten)
    );
  }
      ...
      switchMap((keys) =>
        keys.length
          ? combineLatest(arrayChunk(keys, PAGE_SIZE_V).map(query)).pipe(
              map((valsArr) => arrayFlatten(valsArr).map((value, index) => [keys[index], value]))
            )
          : of([])
      )
    );
