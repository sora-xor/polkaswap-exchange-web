...
export class Init extends Decorate {
  constructor(options, type, decorateMethod) {
    ...
    if (!options.source) {
      ...
      _classPrivateFieldLooseBase(this, _registries)[_registries] = _classPrivateFieldLooseBase(
        options.source,
        _registries
      )[_registries];
    }
    ...
    if (this.hasSubscriptions) {
      this._rpcCore.provider.on('disconnected', () =>
        _classPrivateFieldLooseBase(this, _onProviderDisconnect)[_onProviderDisconnect]()
      );
      this._rpcCore.provider.on('error', (e) =>
        _classPrivateFieldLooseBase(this, _onProviderError)[_onProviderError](e)
      );
  ...
  _initRegistry(registry, chain, version, metadata, chainProps) {
    ...
    registry.setMetadata(
      metadata,
      undefined,
      objectSpread({}, getSpecExtensions(registry, chain, version.specName), this._options.signedExtensions)
    );
  }
  ...
  _getDefaultRegistry() {
    return assertReturn(
      _classPrivateFieldLooseBase(this, _registries)[_registries].find(({ isDefault }) => isDefault),
  ...
  async at(blockHash, knownVersion) {
    ...
    if (
      !_classPrivateFieldLooseBase(this, _atLast)[_atLast] ||
      _classPrivateFieldLooseBase(this, _atLast)[_atLast][0] !== u8aHex
    ) {
  ...
  _cacheBlockRegistryProgress(key, creator) {
    ...
    if (isUndefined(waiting)) {
      ...
      waiting = _classPrivateFieldLooseBase(this, _waitingRegistries)[_waitingRegistries][key] = new Promise(
        (resolve, reject) => {
          creator()
            .then((registry) => {
  ...
  async getBlockRegistry(blockHash, knownVersion) {
    return (
      ...
      _classPrivateFieldLooseBase(this, _registries)[_registries].find(
        ({ lastBlockHash }) => lastBlockHash && u8aEq(lastBlockHash, blockHash)
      ) ||
      // try to find via version
  ...
  _subscribeUpdates() {
    ...
    _classPrivateFieldLooseBase(this, _updateSub)[_updateSub] = this._rpcCore.state
  ...
  _subscribeHealth() {
    ...
    _classPrivateFieldLooseBase(this, _healthTimer)[_healthTimer] = this.hasSubscriptions
      ? setInterval(() => {
