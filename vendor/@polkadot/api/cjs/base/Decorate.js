...
class Decorate extends _Events.Events {
  ...
  constructor(options, type, decorateMethod) {
    ...
    this.#registry =
      ...
      options.registry ||
      new _types.TypeRegistry();
    this._rx.callAt = (blockHash, knownVersion) =>
      (0, _rxjs.from)(this.at(blockHash, knownVersion)).pipe((0, _rxjs.map)((a) => a.rx.call));
    this._rx.queryAt = (blockHash, knownVersion) =>
      (0, _rxjs.from)(this.at(blockHash, knownVersion)).pipe((0, _rxjs.map)((a) => a.rx.query));
    this._rx.registry = this.#registry;
    const thisProvider = options.source
      ? options.source._rpcCore.provider.isClonable
        ? options.source._rpcCore.provider.clone()
        : options.source._rpcCore.provider
      : options.provider || new _rpcProvider.WsProvider();
  ...
  _decorateCalls(_ref3, decorateMethod, blockHash) {
    let {
      registry,
      runtimeVersion: { apis, specName, specVersion },
    } = _ref3;
    const result = {};
    const named = {};
    const hashes = {};
    ...
    const lazySection = (section) =>
      (0, _util.lazyMethods)({}, Object.keys(named[section]), (method) =>
  ...
  _decorateCall(registry, def, stateCall, decorateMethod) {
    ...
    const decorated = decorateMethod(function () {
      ...
      const bytes = registry.createType(
        'Raw',
        (0, _util.u8aConcatStrict)(args.map((a, i) => registry.createTypeUnsafe(def.params[i].type, [a]).toU8a()))
      );
  ...
  _decorateExtrinsics(_ref10, decorateMethod) {
    ...
    const lazySection = (section) =>
      (0, _util.lazyMethods)({}, Object.keys(tx[section]), (method) =>
  ...
  _decorateStorage(_ref11, decorateMethod, blockHash) {
    let { query, registry } = _ref11;
    const result = {};
    const lazySection = (section) =>
      (0, _util.lazyMethods)({}, Object.keys(query[section]), (method) =>
        blockHash
  ...
  _decorateStorageEntry(creator, decorateMethod) {
    ...
    const getQueryAt = (blockHash) =>
      (0, _rxjs.from)(this.at(blockHash)).pipe((0, _rxjs.map)((api) => getAtQueryFn(api, creator)));
    ...
    decorated.sizeAt = decorateMethod(function (blockHash) {
      ...
      return getQueryAt(blockHash).pipe(
        (0, _rxjs.switchMap)((q) =>
          _this2._rpcCore.state.getStorageSize(getArgs(args, q.creator.meta.registry), blockHash)
        )
      );
    });
    ...
    if (creator.iterKey && creator.meta.type.isMap) {
      decorated.entries = decorateMethod(
        (0, _rpcCore.memo)(this.#instanceId, function () {
      ...
      decorated.entriesAt = decorateMethod(
        (0, _rpcCore.memo)(this.#instanceId, function (blockHash) {
          for (
            var _len9 = arguments.length, args = new Array(_len9 > 1 ? _len9 - 1 : 0), _key9 = 1;
            _key9 < _len9;
            _key9++
          ) {
          ...
          return getQueryAt(blockHash).pipe(
            (0, _rxjs.switchMap)((q) => _this2._retrieveMapEntries(q.creator, blockHash, args))
          );
        })
      );
      decorated.entriesPaged = decorateMethod(
        (0, _rpcCore.memo)(this.#instanceId, (opts) => this._retrieveMapEntriesPaged(creator, undefined, opts))
      );
      ...
      decorated.keysAt = decorateMethod(
        (0, _rpcCore.memo)(this.#instanceId, function (blockHash) {
          for (
            var _len11 = arguments.length, args = new Array(_len11 > 1 ? _len11 - 1 : 0), _key11 = 1;
            _key11 < _len11;
            _key11++
          ) {
          ...
          return getQueryAt(blockHash).pipe(
            (0, _rxjs.switchMap)((q) => _this2._retrieveMapKeys(q.creator, blockHash, args))
          );
        })
      );
      decorated.keysPaged = decorateMethod(
        (0, _rpcCore.memo)(this.#instanceId, (opts) => this._retrieveMapKeysPaged(creator, undefined, opts))
      );
    }
  ...
  _decorateStorageEntryAt(registry, creator, decorateMethod, blockHash) {
    ...
    if (creator.iterKey && creator.meta.type.isMap) {
      ...
      decorated.entriesPaged = decorateMethod(
        (0, _rpcCore.memo)(this.#instanceId, (opts) => this._retrieveMapEntriesPaged(creator, blockHash, opts))
      );
      decorated.keys = decorateMethod(
        (0, _rpcCore.memo)(this.#instanceId, function () {
      ...
      decorated.keysPaged = decorateMethod(
        (0, _rpcCore.memo)(this.#instanceId, (opts) => this._retrieveMapKeysPaged(creator, blockHash, opts))
      );
    }
    if (this.supportMulti && creator.meta.type.isMap) {
      // When using double map storage function, user need to pass double map key as an array
  ...
  _decorateStorageCall(creator, decorateMethod) {
    ...
    return decorateMethod(memoed, {
      ...
      overrideNoSub: function () {
        ...
        return _this4._queueStorage(
          (0, _validate.extractStorageArgs)(_this4.#registry, creator, args),
          _this4.#storageGetQ
        );
      },
    });
  ...
  _retrieveMulti(keys, blockHash) {
    ...
    return (0, _rxjs.combineLatest)(
      (0, _util.arrayChunk)(keys, PAGE_SIZE_V).map((k) => (blockHash ? query(k, blockHash) : query(k)))
    ).pipe((0, _rxjs.map)(_util.arrayFlatten));
  }
  _retrieveMapKeys(_ref12, at, args) {
    let { iterKey, meta, method, section } = _ref12;
    ...
    return startSubject.pipe(
      (0, _rxjs.switchMap)(query),
      (0, _rxjs.map)((keys) => keys.map(setMeta)),
      (0, _rxjs.tap)((keys) =>
        (0, _util.nextTick)(() => {
  ...
  _retrieveMapKeysPaged(_ref13, at, opts) {
    let { iterKey, meta, method, section } = _ref13;
    if (!iterKey || !meta.type.isMap) {
      throw new Error('keys can only be retrieved on maps');
    }
    const setMeta = (key) => key.setMeta(meta, section, method);
  ...
  _retrieveMapEntries(entry, at, args) {
    ...
    return this._retrieveMapKeys(entry, at, args).pipe(
      (0, _rxjs.switchMap)((keys) =>
        keys.length
          ? (0, _rxjs.combineLatest)((0, _util.arrayChunk)(keys, PAGE_SIZE_V).map(query)).pipe(
              (0, _rxjs.map)((valsArr) => (0, _util.arrayFlatten)(valsArr).map((value, index) => [keys[index], value]))
            )
          : (0, _rxjs.of)([])
      )
    );
  ...
  _retrieveMapEntriesPaged(entry, at, opts) {
    ...
    return this._retrieveMapKeysPaged(entry, at, opts).pipe(
      (0, _rxjs.switchMap)((keys) =>
        keys.length
          ? query(keys).pipe((0, _rxjs.map)((valsArr) => valsArr.map((value, index) => [keys[index], value])))
          : (0, _rxjs.of)([])
      )
    );
  }
  _decorateDeriveRx(decorateMethod) {
    ...
    const available = (0, _apiDerive.getAvailableDerives)(
      this.#instanceId,
      this._rx,
