...
class Init extends _Decorate.Decorate {
  constructor(options, type, decorateMethod) {
    ...
    if (!options.source) {
      ...
      (0, _classPrivateFieldLooseBase2.default)(this, _registries)[_registries] = (0,
      _classPrivateFieldLooseBase2.default)(options.source, _registries)[_registries];
    }
    ...
    this._rpcCore.setResolveBlockHash((blockNumber) =>
      (0, _rxjs.firstValueFrom)(this._rpcCore.chain.getBlockHash(blockNumber))
    );
    if (this.hasSubscriptions) {
      this._rpcCore.provider.on('disconnected', () =>
        (0, _classPrivateFieldLooseBase2.default)(this, _onProviderDisconnect)[_onProviderDisconnect]()
      );
      this._rpcCore.provider.on('error', (e) =>
        (0, _classPrivateFieldLooseBase2.default)(this, _onProviderError)[_onProviderError](e)
      );
  ...
  _initRegistry(registry, chain, version, metadata, chainProps) {
    ...
    registry.setMetadata(
      ...
      (0, _util.objectSpread)(
        {},
        (0, _typesKnown.getSpecExtensions)(registry, chain, version.specName),
  ...
  _getDefaultRegistry() {
    return (0, _util.assertReturn)(
      (0, _classPrivateFieldLooseBase2.default)(this, _registries)[_registries].find((_ref) => {
        let { isDefault } = _ref;
        return isDefault;
      }),
  ...
  async at(blockHash, knownVersion) {
    ...
    if (
      !(0, _classPrivateFieldLooseBase2.default)(this, _atLast)[_atLast] ||
      (0, _classPrivateFieldLooseBase2.default)(this, _atLast)[_atLast][0] !== u8aHex
    ) {
  ...
  async _createBlockRegistry(blockHash, header, version) {
    ...
    const metadata = new _types.Metadata(
      registry,
      await (0, _rxjs.firstValueFrom)(this._rpcCore.state.getMetadata.raw(header.parentHash))
    );
  ...
  _cacheBlockRegistryProgress(key, creator) {
    ...
    if ((0, _util.isUndefined)(waiting)) {
      ...
      waiting = (0, _classPrivateFieldLooseBase2.default)(this, _waitingRegistries)[_waitingRegistries][key] =
  ...
  async _getBlockRegistryViaHash(blockHash) {
    ...
    const version = this.registry.createType(
      ...
      WITH_VERSION_SHORTCUT &&
        firstVersion &&
        (lastVersion || firstVersion.specVersion.eq(this._runtimeVersion.specVersion))
        ? {
  ...
  async _metaFromSource(source) {
    ...
    this._filterRpc(
      rpcs,
      (0, _typesKnown.getSpecRpc)(this.registry, source.runtimeChain, source.runtimeVersion.specName)
    );
  ...
  _subscribeUpdates() {
    ...
    (0, _classPrivateFieldLooseBase2.default)(this, _updateSub)[_updateSub] = this._rpcCore.state
      .subscribeRuntimeVersion()
      .pipe(
        (0, _rxjs.switchMap)((version) => {
          var _this$_runtimeVersion;
          return (
            // only retrieve the metadata when the on-chain version has been changed
            (_this$_runtimeVersion = this._runtimeVersion) != null &&
  ...
  async _metaFromChain(optMetadata) {
    ...
    const metadata =
      ...
      (optMetadata && optMetadata[metadataKey]
        ? new _types.Metadata(this.registry, optMetadata[metadataKey])
        : await (0, _rxjs.firstValueFrom)(this._rpcCore.state.getMetadata()));

    // initializes the registry & RPC
    this._initRegistry(this.registry, chain, runtimeVersion, metadata, chainProps);
    this._filterRpc(
      rpcMethods.methods.map(textToString),
      (0, _typesKnown.getSpecRpc)(this.registry, chain, runtimeVersion.specName)
    );
  ...
  _subscribeHealth() {
    ...
    (0, _classPrivateFieldLooseBase2.default)(this, _healthTimer)[_healthTimer] = this.hasSubscriptions
      ? setInterval(() => {
