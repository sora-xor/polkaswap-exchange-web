    ...
    if (queryAt['authorMapping']?.['mappingWithDeposit']) {
      return combineLatest([
        ...
        queryAt['authorMapping']
          ...
          .pipe(map((o) => o.unwrapOr({ account: null }).account)),
      ]);
    }
          ...
          mergeMap((selectedCandidates) =>
            combineLatest([
              ...
              queryAt.session.nextKeys
                ...
                .pipe(map((nextKeys) => nextKeys.findIndex((o) => o.unwrapOrDefault().nimbus.toHex() === loggedHex))),
            ])
          ),
...
export function getAuthorDetails(api, header, blockHash) {
  ...
  return api
    ...
    .pipe(switchMap((queryAt) => getAuthorDetailsWithAt(header, queryAt)));
}
