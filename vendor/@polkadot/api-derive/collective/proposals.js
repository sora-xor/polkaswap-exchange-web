...
function _proposalsFrom(api, query, hashes) {
  return (
    isFunction(query?.proposals) && hashes.length
      ? combineLatest([
          of(hashes),
...
export function proposals(section) {
  return withSection(
    section,
    (query, api) => () => api.derive[section].proposalHashes().pipe(switchMap((all) => _proposalsFrom(api, query, all)))
  );
}
export function proposal(section) {
  return withSection(
    section,
    (query, api) => (hash) =>
      isFunction(query?.proposals) ? firstObservable(_proposalsFrom(api, query, [hash])) : of(null)
  );
}
