  ...
  if (Array.isArray(locks)) {
    ...
    vestingLocked = api.registry.createType(
      ...
      lockedBreakdown
        ...
        .reduce((result, { amount }) => result.iadd(amount), new BN(0))
    );
...
function calcShared(api, bestNumber, data, locks) {
  ...
  if (data?.frameSystemAccountInfo?.frozen) {
    ...
    transferable = api.registry.createType(
      'Balance',
      allLocked ? 0 : bnMax(new BN(0), freeBalance.sub(bnMax(maybeED, frozenReserveDif)))
    );
  }
  return objectSpread({}, data, {
    availableBalance: api.registry.createType(
      'Balance',
      allLocked ? 0 : bnMax(new BN(0), data?.freeBalance ? data.freeBalance.sub(lockedBalance) : new BN(0))
    ),
    lockedBalance,
    lockedBreakdown,
    transferable,
