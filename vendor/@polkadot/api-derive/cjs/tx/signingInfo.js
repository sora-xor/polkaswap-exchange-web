...
function signingHeader(api) {
  return (0, rxjs_1.combineLatest)([
    ...
    api.rpc.chain
      .getFinalizedHead()
      .pipe(
        (0, rxjs_1.switchMap)((hash) =>
          api.rpc.chain.getHeader(hash).pipe((0, rxjs_1.catchError)(() => (0, rxjs_1.of)(null)))
        )
      ),
  ]).pipe(
    (0, rxjs_1.map)(([current, finalized]) =>
      // determine the hash to use, current when lag > max, else finalized
      !finalized ||
      (0, index_js_1.unwrapBlockNumber)(current)
        .sub((0, index_js_1.unwrapBlockNumber)(finalized))
        .gt(constants_js_1.MAX_FINALITY_LAG)
        ? current
        : finalized
    )
  );
...
function signingInfo(_instanceId, api) {
  ...
  return (address, nonce, era) =>
    (0, rxjs_1.combineLatest)([
      ...
      (0, rxjs_1.map)(([nonce, header]) => ({
        header,
        mortalLength: Math.min(
          api.consts.system?.blockHashCount?.toNumber() || constants_js_1.FALLBACK_MAX_HASH_COUNT,
          constants_js_1.MORTAL_PERIOD.div(babeOrAuraPeriod(api) || constants_js_1.FALLBACK_PERIOD)
