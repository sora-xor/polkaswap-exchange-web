...
function _membersPrev(api, accountIds) {
  return (0, rxjs_1.combineLatest)([
    ...
    (0, rxjs_1.map)(([accountIds, payouts, strikes, defenderVotes, suspended, vouching]) =>
...
function _membersCurr(api, accountIds) {
  return (0, rxjs_1.combineLatest)([
    ...
    api.query.society
      .challengeRoundCount()
      .pipe(
        (0, rxjs_1.switchMap)((round) =>
          api.query.society.defenderVotes.multi(accountIds.map((accountId) => [round, accountId]))
        )
      ),
    api.query.society.suspendedMembers.multi(accountIds),
  ]).pipe(
    (0, rxjs_1.map)(([accountIds, members, payouts, defenderVotes, suspendedMembers]) =>
      accountIds
        .map((accountId, index) =>
...
function _members(instanceId, api) {
  return (0, index_js_1.memo)(instanceId, (accountIds) =>
    api.query.society.members.creator.meta.type.isMap ? _membersCurr(api, accountIds) : _membersPrev(api, accountIds)
  );
}
/**
 * @name members
  ...
  return (0, index_js_1.memo)(instanceId, () =>
    api.query.society.members.creator.meta.type.isMap
      ? api.query.society.members
          .keys()
          .pipe(
            (0, rxjs_1.switchMap)((keys) => api.derive.society._members(keys.map(({ args: [accountId] }) => accountId)))
          )
