  ...
  if (Array.isArray(locks)) {
    ...
    vestingLocked = api.registry.createType(
      ...
      lockedBreakdown
        ...
        .reduce((result, { amount }) => result.iadd(amount), new util_1.BN(0))
    );
...
function calcShared(api, bestNumber, data, locks) {
  ...
  if (data?.frameSystemAccountInfo?.frozen) {
    ...
    transferable = api.registry.createType(
      'Balance',
      allLocked ? 0 : (0, util_1.bnMax)(new util_1.BN(0), freeBalance.sub((0, util_1.bnMax)(maybeED, frozenReserveDif)))
    );
  }
  return (0, util_1.objectSpread)({}, data, {
    availableBalance: api.registry.createType(
      ...
      allLocked
        ...
        : (0, util_1.bnMax)(
            new util_1.BN(0),
            data?.freeBalance ? data.freeBalance.sub(lockedBalance) : new util_1.BN(0)
          )
    ),
    lockedBalance,
    lockedBreakdown,
...
function queryCurrent(api, accountId, balanceInstances = ['balances']) {
  const [lockEmpty, lockQueries] = createCalls(
    balanceInstances.map((m) => api.derive[m]?.customLocks || api.query[m]?.locks)
  );
  ...
  return (0, rxjs_1.combineLatest)([
    ...
    lockQueries.length ? (0, rxjs_1.combineLatest)(lockQueries.map((c) => c(accountId))) : (0, rxjs_1.of)([]),
    reserveQueries.length ? (0, rxjs_1.combineLatest)(reserveQueries.map((c) => c(accountId))) : (0, rxjs_1.of)([]),
  ]).pipe(
    (0, rxjs_1.map)(([opt, locks, reserves]) => {
      let offsetLock = -1;
      let offsetReserve = -1;
...
function all(instanceId, api) {
  ...
  return (0, index_js_1.memo)(instanceId, (address) =>
    (0, rxjs_1.combineLatest)([
      ...
      (0, rxjs_1.switchMap)(([account, locks]) =>
        (0, rxjs_1.combineLatest)([(0, rxjs_1.of)(account), (0, rxjs_1.of)(locks), api.derive.chain.bestNumber()])
      ),
