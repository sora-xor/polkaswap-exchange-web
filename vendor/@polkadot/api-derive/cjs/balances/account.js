...
function queryBalancesFree(api, accountId) {
  return (0, rxjs_1.combineLatest)([
    ...
    (0, rxjs_1.map)(([freeBalance, reservedBalance, accountNonce]) => [
      accountNonce,
...
function queryBalancesAccount(api, accountId, modules = ['balances']) {
  ...
  return balances.length
    ? (0, util_1.isFunction)(api.query.system.account)
      ? (0, rxjs_1.combineLatest)([api.query.system.account(accountId), ...balances.map((c) => c(accountId))]).pipe(
          (0, rxjs_1.map)(([{ nonce }, ...balances]) => extract(nonce, balances))
        )
  ...
  return api.query.system.account(accountId).pipe(
    (0, rxjs_1.map)((infoOrTuple) => {
      ...
      if (isFrameType) {
        ...
        return [nonce, [[free, reserved, frozen, flags]], { isFrameAccountData: true }];
      } else {
        const { feeFrozen, free, miscFrozen, reserved } = data;
        return [nonce, [[free, reserved, feeFrozen, miscFrozen]], { isFrameAccountData: false }];
      }
    })
  );
}
