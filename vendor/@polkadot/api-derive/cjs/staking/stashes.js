...
function onBondedEvent(api) {
  ...
  return api.query.system.events().pipe(
    ...
    (0, rxjs_1.startWith)(current),
    (0, index_js_1.drr)({ skipTimeout: true })
  );
}
/**
 * @name stashes
...
function stashes(instanceId, api) {
  return (0, index_js_1.memo)(instanceId, () =>
    onBondedEvent(api).pipe(
      (0, rxjs_1.switchMap)(() => api.query.staking.validators.keys()),
      (0, rxjs_1.map)((keys) => keys.map(({ args: [v] }) => v).filter((a) => a))
    )
  );
}
