      ...
      if (!(expTotal.isZero() || valPoints.isZero() || eraPoints.isZero())) {
        ...
        if (validatorId === stakerId) {
          if (exposure.own) {
            ...
            staked = expAccount ? expAccount.value.unwrap() : util_1.BN_ZERO;
          }
        } else {
          const stakerExp = exposure.others.find(({ who }) => who.eq(stakerId));
          staked = stakerExp ? stakerExp.value.unwrap() : util_1.BN_ZERO;
        }
        value = avail
          .sub(valCut)
          .imul(staked)
          .div(expTotal)
          .iadd(validatorId === stakerId ? valCut : util_1.BN_ZERO);
      }
...
function _stakerRewardsEras(instanceId, api) {
  return (0, index_js_1.memo)(instanceId, (eras, withActive = false) =>
...
function _stakerRewards(instanceId, api) {
  return (0, index_js_1.memo)(instanceId, (accountIds, eras, withActive = false) => {
    ...
    const sanitizedEras = eras.map((e) =>
      typeof e === 'number' || typeof e === 'string' ? api.registry.createType('u32', e) : e
    );
    return (0, rxjs_1.combineLatest)([
      ...
      (0, rxjs_1.switchMap)(([queries, exposures, erasResult]) => {
        const allRewards = queries.map(({ claimedRewardsEras, stakingLedger, stashId }, index) =>
        ...
        return api.derive.staking.queryMulti(allValidators, { withClaimedRewardsEras: true, withLedger: true }).pipe(
          (0, rxjs_1.map)((queriedVals) =>
            queries.map(({ claimedRewardsEras, stakingLedger }, index) =>
              filterRewards(
                eras,
                stashValidators[index]
                  .map((validatorId) => [validatorId, queriedVals.find((q) => q.accountId.eq(validatorId))])
                  .filter((v) => !!v[1]),
                {
                  claimedRewardsEras,
 ...
 * ```
 */
exports.stakerRewards = (0, index_js_1.firstMemo)((api, accountId, withActive) =>
  api.derive.staking
    .erasHistoric(withActive)
    .pipe((0, rxjs_1.switchMap)((eras) => api.derive.staking._stakerRewards([accountId], eras, withActive)))
);
/**
...
function stakerRewardsMultiEras(instanceId, api) {
  return (0, index_js_1.memo)(instanceId, (accountIds, eras) =>
    accountIds.length && eras.length ? api.derive.staking._stakerRewards(accountIds, eras, false) : (0, rxjs_1.of)([])
  );
}
/**
...
function stakerRewardsMulti(instanceId, api) {
  return (0, index_js_1.memo)(instanceId, (accountIds, withActive = false) =>
    api.derive.staking
      .erasHistoric(withActive)
      .pipe((0, rxjs_1.switchMap)((eras) => api.derive.staking.stakerRewardsMultiEras(accountIds, eras)))
  );
}
