...
function getLedgers(api, optIds, { withLedger = false }) {
  ...
  return (
    ids.length ? (0, rxjs_1.combineLatest)(ids.map((s) => api.query.staking.ledger(s))) : (0, rxjs_1.of)([])
  ).pipe(
    (0, rxjs_1.map)((optLedgers) => {
      let offset = -1;
  ...
  return (0, rxjs_1.combineLatest)([
    ...
    withClaimedRewardsEras && api.query.staking.claimedRewards
      ? (0, rxjs_1.combineLatest)([
          ...
          (0, rxjs_1.map)(([rewardsStorageVec, overviewStorageVec]) =>
            filterRewards(stashIds, eras, rewardsStorageVec, overviewStorageVec)
          )
        )
    ...
    (0, rxjs_1.switchMap)(
      ([
        ...
        getLedgers(api, controllerIdOpt, flags).pipe(
          (0, rxjs_1.map)((stakingLedgerOpts) =>
            stashIds.map((stashId, index) =>
              parseDetails(
                ...
                exposureMeta[index],
                claimedRewardsEras[index],
                exposureEraStakers[index]
              )
            )
          )
        )
 ...
 * ```
 */
exports.query = (0, index_js_1.firstMemo)((api, accountId, flags, page) =>
...
function queryMulti(instanceId, api) {
  return (0, index_js_1.memo)(instanceId, (accountIds, flags, page) =>
    api.derive.session.indexes().pipe(
      (0, rxjs_1.switchMap)(({ activeEra }) => {
