  ...
  return (0, index_js_1.memo)(instanceId, () =>
    ...
    api.query.staking.erasStakersOverview
      ? api.derive.session.indexes().pipe(
          ...
          (0, rxjs_1.map)((keys) =>
            [...new Set(keys.map(({ args: [, accountId] }) => accountId.toString()))].map((a) =>
              api.registry.createType('AccountId', a)
            )
          )
        )
...
function validators(instanceId, api) {
  return (0, index_js_1.memo)(instanceId, () =>
    ...
    (0, rxjs_1.combineLatest)([
      api.query.session ? api.query.session.validators() : (0, rxjs_1.of)([]),
      api.query.staking ? api.derive.staking.nextElected() : (0, rxjs_1.of)([]),
    ]).pipe(
      (0, rxjs_1.map)(([validators, nextElected]) => ({
