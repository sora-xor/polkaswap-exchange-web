...
function schedulerEntries(api) {
  ...
  return api.derive.democracy.referendumsFinished().pipe(
    (0, rxjs_1.switchMap)(() => api.query.scheduler.agenda.keys()),
    (0, rxjs_1.switchMap)((keys) => {
      ...
      return blockNumbers.length
        ? (0, rxjs_1.combineLatest)([
            ...
            api.query.scheduler.agenda
              .multi(blockNumbers)
              .pipe((0, rxjs_1.catchError)(() => (0, rxjs_1.of)(blockNumbers.map(() => [])))),
          ])
        : (0, rxjs_1.of)([[], []]);
    })
  );
...
function queryScheduler(api) {
  return schedulerEntries(api).pipe(
    (0, rxjs_1.switchMap)(([blockNumbers, agendas]) => {
      const result = [];
      ...
      return (0, rxjs_1.combineLatest)([
        (0, rxjs_1.of)(result),
        result.length ? api.derive.democracy.preimages(result.map(({ imageHash }) => imageHash)) : (0, rxjs_1.of)([]),
      ]);
    }),
    (0, rxjs_1.map)(([infos, images]) =>
      infos.map((info, index) => (0, util_1.objectSpread)({ image: images[index] }, info))
    )
  );
}
/**
...
function dispatchQueue(instanceId, api) {
  return (0, index_js_1.memo)(instanceId, () =>
    (0, util_1.isFunction)(api.query.scheduler?.agenda)
      ? queryScheduler(api)
