...
function getDemocracyImages(api, bounded) {
  ...
  return api.query.democracy['preimages']
    .multi(hashes)
    .pipe((0, rxjs_1.map)((images) => images.map((imageOpt) => parseDemocracy(api, imageOpt))));
}
...
function preimages(instanceId, api) {
  return (0, index_js_1.memo)(instanceId, (hashes) =>
    hashes.length
