...
function retrievePrev(api, elections) {
  return (0, rxjs_1.combineLatest)([retrieveStakeOf(elections), retrieveVoteOf(elections)]).pipe(
    (0, rxjs_1.map)(([stakes, votes]) => {
      const result = [];
...
function votes(instanceId, api) {
  ...
  return (0, index_js_1.memo)(instanceId, () =>
    elections ? (elections['stakeOf'] ? retrievePrev(api, elections) : retrieveCurrent(elections)) : (0, rxjs_1.of)([])
  );
}
