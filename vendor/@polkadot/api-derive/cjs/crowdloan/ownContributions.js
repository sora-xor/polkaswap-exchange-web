...
function _getValues(api, childKey, keys) {
  ...
  return (0, rxjs_1.combineLatest)(keys.map((k) => api.rpc.childstate.getStorage(childKey, k))).pipe(
    (0, rxjs_1.map)((values) =>
      values
...
function _watchOwnChanges(api, paraId, childkey, keys) {
  return api.query.system.events().pipe(
    (0, rxjs_1.switchMap)((events) => {
      ...
      return filtered.length ? _getValues(api, childkey, filtered) : rxjs_1.EMPTY;
    }),
    (0, rxjs_1.startWith)({})
  );
}
...
function ownContributions(instanceId, api) {
  return (0, index_js_1.memo)(instanceId, (paraId, keys) =>
    api.derive.crowdloan
      .childKey(paraId)
      .pipe(
        (0, rxjs_1.switchMap)((childKey) =>
          childKey && keys.length ? _contributions(api, paraId, childKey, keys) : (0, rxjs_1.of)({})
        )
      )
  );
}
