...
function _proposalsFrom(api, query, hashes) {
  return (
    (0, util_1.isFunction)(query?.proposals) && hashes.length
      ...
      : (0, rxjs_1.of)([[], [], []])
  ).pipe((0, rxjs_1.map)((r) => parse(api, r)));
}
function hasProposals(section) {
  return (0, helpers_js_1.withSection)(
    section,
    (query) => () => (0, rxjs_1.of)((0, util_1.isFunction)(query?.proposals))
  );
}
function proposals(section) {
  return (0, helpers_js_1.withSection)(
    section,
    (query, api) => () =>
      api.derive[section].proposalHashes().pipe((0, rxjs_1.switchMap)((all) => _proposalsFrom(api, query, all)))
  );
}
function proposal(section) {
  return (0, helpers_js_1.withSection)(
    section,
    (query, api) => (hash) =>
      (0, util_1.isFunction)(query?.proposals)
