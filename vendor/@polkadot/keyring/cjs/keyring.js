...
class Keyring {
  ...
  createFromJson(_ref2, ignoreChecksum) {
    ...
    const publicKey = (0, _util.isHex)(address)
      ? (0, _util.hexToU8a)(address)
  ...
  createFromUri(_suri) {
    ...
    if (isPhraseHex) {
      ...
      if ([12, 15, 18, 21, 24].includes(parts.length)) {
        seed =
          type === 'ethereum'
            ? (0, _utilCrypto.mnemonicToLegacySeed)(phrase, '', false, 64)
            : (0, _utilCrypto.mnemonicToMiniSecret)(phrase, password);
      } else {
        if (phrase.length > 32) {
    ...
    const derived =
      type === 'ethereum'
        ? isPhraseHex
          ...
          : (0, _utilCrypto.hdEthereum)(seed, derivePath.substring(1))
        : (0, _utilCrypto.keyFromPath)(PairFromSeed[type](seed), path, type);
    return (0, _pair.createPair)(
      {
        toSS58: this.encodeAddress,
        type,
  ...
  encodeAddress = (address, ss58Format) => {
    return this.type === 'ethereum'
      ? (0, _utilCrypto.ethereumEncode)(address)
      : (0, _utilCrypto.encodeAddress)(address, ss58Format === undefined ? this.#ss58 : ss58Format);
  };
