...
function checkInstance(created, matcher) {
  ...
  if (!isOk) {
    throw new Error(
      `${rawType}:: Decoded input doesn't match input, received ${(0, _util.u8aToHex)(matcher, 512)} (${matcher.length} bytes), created ${(0, _util.u8aToHex)(u8a, 512)} (${u8a.length} bytes)`
    );
  }
}
...
function initType(registry, Type) {
  ...
  let { blockHash, isFallback, isOptional, isPedantic } =
