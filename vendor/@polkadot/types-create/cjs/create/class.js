...
const infoMapping = {
  ...
  [_types.TypeDefInfo.DoNotConstruct]: (registry, value) =>
    _typesCodec.DoNotConstruct.with(value.displayName || value.type),
  [_types.TypeDefInfo.Enum]: (registry, value) => {
    const subs = getSubDefArray(value);
    return _typesCodec.Enum.with(
      subs.every((_ref3) => {
        let { type } = _ref3;
        return type === 'Null';
      })
        ? subs.reduce((out, _ref4, count) => {
            let { index, name } = _ref4;
  ...
  [_types.TypeDefInfo.Set]: (registry, value) =>
    _typesCodec.CodecSet.with(
      getSubDefArray(value).reduce((result, _ref5) => {
        let { index, name } = _ref5;
        result[name] = index;
        return result;
      }, {}),
      value.length
  ...
  [_types.TypeDefInfo.VecFixed]: (registry, _ref7) => {
    let { displayName, length, sub } = _ref7;
    if (!(0, _util.isNumber)(length) || !sub || Array.isArray(sub)) {
      throw new Error('Expected length & type information for fixed vector');
    }
    return sub.type === 'u8'
      ? _typesCodec.U8aFixed.with(length * 8, displayName)
      : _typesCodec.VecFixed.with(getTypeDefType(sub), length);
  },
...
function createClassUnsafe(registry, type) {
  return (
    ...
    getTypeClass(
      registry,
      registry.isLookupType(type) ? registry.lookup.getTypeDef(type) : (0, _getTypeDef.getTypeDef)(type)
    )
  );
}
