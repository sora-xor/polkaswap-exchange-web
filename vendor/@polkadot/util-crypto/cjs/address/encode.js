...
function encodeAddress(key, ss58Format = defaults_js_1.defaults.prefix) {
  ...
  if (ss58Format < 0 || (ss58Format > 16383 && !ss58Exceptions.includes(ss58Format)) || [46, 47].includes(ss58Format)) {
    ...
    throw new Error(
      `Expected a valid key to convert, with length ${defaults_js_1.defaults.allowedDecodedLengths.join(', ')}`
    );
  }
  const input = (0, util_1.u8aConcat)(
    ss58Format < 64
      ? [ss58Format]
      : [
  ...
  return (0, index_js_1.base58Encode)(
    (0, util_1.u8aConcat)(input, (0, sshash_js_1.sshash)(input).subarray(0, [32, 33].includes(u8a.length) ? 2 : 1))
  );
}
