...
function hdLedger(_mnemonic, path, rounds) {
  ...
  let seed = (0, master_js_1.ledgerMaster)(mnemonic, password, rounds);
  for (const p of parts) {
    const n = parseInt(p.replace(/'$/, ''), 10);
    seed = (0, derivePrivate_js_1.ledgerDerivePrivate)(
      seed,
      n < validatePath_js_1.HARDENED ? n + validatePath_js_1.HARDENED : n
    );
  }
