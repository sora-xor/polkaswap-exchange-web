...
export type __Codec<V extends Codec[]> = V[0] extends Codec ? V[0] : Codec;
export type __Sanitize<K extends string> = K extends ` ${infer X}` | `${infer X} ` | ` ${infer X} `
  ? __Sanitize<X>
...
export type __ToTuple<O extends Codec[]> = O[0] extends Codec ? (O[1] extends Codec ? ITuple<O> : O[0]) : Null;
export type __CodecsNext<K, C extends Codec[]> = K extends __WrapOne
...
export type __Codecs<T extends unknown[]> = T extends [infer K, ...infer N] ? __CodecsNext<K, __Codecs<N>> : [];
export type __Combine<V extends __Value[], I extends string> = I extends '' ? V : [...V, I];
export type __CombineInner<V extends __Value[], I extends string, X extends __Value> = I extends ''
  ? [...V, X]
  : [I, ...V, X];
export type __TokenizeStruct<
  T extends [__Value[], string],
  V extends __Value[],
  I extends string,
  R extends string,
> = __Tokenize<
  ...
  >
>;
export type __TokenizeTuple<T extends [__Value[], string], V extends __Value[], I extends string> = __Tokenize<
  T[1],
  __CombineInner<V, I, T[0]>
>;
export type __Tokenize<K extends string, V extends __Value[] = [], I extends string = ''> = K extends
  | ''
  | '>'
  | ')'
  | '}'
