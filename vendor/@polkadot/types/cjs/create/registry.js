...
function injectErrors(_, _ref2, version, result) {
  let { lookup, pallets } = _ref2;
  clearRecord(result);
  for (let i = 0; i < pallets.length; i++) {
    const { errors, index, name } = pallets[i];
    if (errors.isSome) {
      const sectionName = (0, _util.stringCamelCase)(name);
      (0, _util.lazyMethod)(result, version >= 12 ? index.toNumber() : i, () =>
        (0, _lazy.lazyVariants)(lookup, errors.unwrap(), getVariantStringIdx, (_ref3) => {
          let { docs, fields, index, name } = _ref3;
          return {
...
function injectEvents(registry, _ref4, version, result) {
  let { lookup, pallets } = _ref4;
  const filtered = pallets.filter(_decorate.filterEventsSome);
  clearRecord(result);
  for (let i = 0; i < filtered.length; i++) {
    const { events, index, name } = filtered[i];
    (0, _util.lazyMethod)(result, version >= 12 ? index.toNumber() : i, () =>
      (0, _lazy.lazyVariants)(lookup, events.unwrap(), getVariantStringIdx, (variant) => {
        const meta = registry.createType(
          'EventMetadataLatest',
          (0, _util.objectSpread)({}, variant, {
...
function injectExtrinsics(registry, _ref5, version, result, mapping) {
  ...
  for (let i = 0; i < filtered.length; i++) {
    ...
    (0, _util.lazyMethod)(result, sectionIndex, () =>
      (0, _lazy.lazyVariants)(lookup, allCalls, getVariantStringIdx, (variant) =>
        (0, _extrinsics.createCallFunction)(registry, lookup, variant, sectionName, sectionIndex)
      )
    );
    ...
    if (palletIdx !== -1) {
      const name = (0, _util.stringCamelCase)(
        path
          .slice(0, palletIdx)
          .map((p, i) =>
            i === 0
...
class TypeRegistry {
  ...
  #registerLookup = (lookup) => {
    ...
    if (this.hasType('SpWeightsWeightV2Weight')) {
      ...
      Weight =
        weightv2.refTime && weightv2.proofSize
  ...
  setMetadata(metadata, signedExtensions, userExtensions) {
    ...
    this.setSignedExtensions(
      signedExtensions ||
        (this.#metadata.extrinsic.version.gt(_util.BN_ZERO)
          ? // FIXME Use the extension and their injected types
            this.#metadata.extrinsic.signedExtensions.map((_ref6) => {
              let { identifier } = _ref6;
              return identifier.toString();
            })
          : _signedExtensions.fallbackExtensions),
      userExtensions
    );
  ...
  setSignedExtensions() {
    let signedExtensions =
      arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : _signedExtensions.fallbackExtensions;
