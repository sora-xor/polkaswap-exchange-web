...
function assertArgs(_ref, _ref2) {
  ...
  if (!Array.isArray(args)) {
    throw new Error(
      `Call to ${(0, _util.stringCamelCase)(section || 'unknown')}.${(0, _util.stringCamelCase)(method || 'unknown')} needs ${keys.length} arguments`
    );
  } else if (args.filter(filterDefined).length !== keys.length) {
    throw new Error(
      `Call to ${(0, _util.stringCamelCase)(section || 'unknown')}.${(0, _util.stringCamelCase)(method || 'unknown')} needs ${keys.length} arguments, found [${args.join(', ')}]`
    );
  }
}
...
function createKeyRawParts(registry, itemFn, _ref3) {
  let { args, hashers, keys } = _ref3;
  const extra = new Array(keys.length);
  for (let i = 0; i < keys.length; i++) {
    extra[i] = (0, _getHasher.getHasher)(hashers[i])(
...
function createKeyInspect(registry, itemFn, args) {
  ...
  if (meta.type.isMap) {
    ...
    types =
      hashers.length === 1
        ? [`${hashers[0].type}(${(0, _util2.getSiName)(registry.lookup, key)})`]
        : registry.lookup
            .getSiType(key)
            .def.asTuple.map((k, i) => `${hashers[i].type}(${(0, _util2.getSiName)(registry.lookup, k)})`);
  }
    ...
    if (type.isPlain) {
      if (!cacheKey) {
        cacheKey = options.skipHashing
          ? (0, _util.compactAddLength)((0, _util.u8aToU8a)(options.key))
...
function extendPrefixedMap(registry, itemFn, storageFn) {
  ...
  storageFn.iterKey = extendHeadMeta(registry, itemFn, storageFn, function () {
    ...
    if (args.length && (type.isPlain || args.length >= type.asMap.hashers.length)) {
      throw new Error(
        `Iteration of ${(0, _util.stringCamelCase)(section || 'unknown')}.${(0, _util.stringCamelCase)(method || 'unknown')} needs arguments to be at least one less than the full arguments, found [${args.join(', ')}]`
      );
    }
    if (args.length) {
      if (type.isMap) {
