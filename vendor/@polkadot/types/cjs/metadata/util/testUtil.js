...
function writeJson(json, version, type, sub) {
  _fs.default.writeFileSync(
    _path.default.join(process.cwd(), `packages/types-support/src/metadata/v${version}/${type}-${sub}.json`),
    (0, _util.stringify)(json, 2),
    {
      flag: 'w',
    }
  );
            ...
            expect(() => {
              try {
                const instance = registry.createTypeUnsafe(
                  registry.createLookupType((0, _StorageKey.unwrapStorageSi)(type)),
                  [(0, _util.hexToU8a)(fallback.toHex())],
                  {
                    isOptional: modifier.isOptional,
                  }
                );
                if (withFallbackCheck) {
                  const [hexType, hexOrig] = [
                    (0, _util.u8aToHex)(instance.toU8a()),
                    (0, _util.u8aToHex)(fallback.toU8a(true)),
                  ];
                  if (hexType !== hexOrig) {
                    throw new Error(
                      `Fallback does not match (${(hexOrig.length - 2) / 2 - (hexType.length - 2) / 2} bytes missing): ${hexType} !== ${hexOrig}`
                    );
                  }
                }
              } catch (error) {
