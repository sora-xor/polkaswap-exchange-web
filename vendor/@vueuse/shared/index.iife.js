(function (exports, vue) {
  ...
  const injectLocal = /* @__NO_SIDE_EFFECTS__ */ (...args) => {
    ...
    if (instance == null && !vue.hasInjectionContext()) throw new Error('injectLocal must be called in setup');
  ...
  function get(obj, key) {
    if (key == null) return vue.unref(obj);
  ...
  function toReactive(objectRef) {
    if (!vue.isRef(objectRef)) return vue.reactive(objectRef);
    const proxy = new Proxy(
      {},
      {
        ...
        set(_, p, value) {
          if (vue.isRef(objectRef.value[p]) && !vue.isRef(value)) objectRef.value[p].value = value;
  ...
  function reactiveOmit(obj, ...keys) {
    ...
    return reactiveComputed(() =>
      typeof predicate === 'function'
        ? Object.fromEntries(Object.entries(vue.toRefs(obj)).filter(([k, v]) => !predicate(vue.toValue(v), k)))
        : Object.fromEntries(Object.entries(vue.toRefs(obj)).filter((e) => !flatKeys.includes(e[0])))
    );
  }
  ...
  function syncRefs(source, targets, options = {}) {
    ...
    return vue.watch(source, (newValue) => targetsArray.forEach((target) => (target.value = newValue)), {
      flush,
      deep,
      immediate,
    });
  ...
  function tryOnBeforeMount(fn, sync = true, target) {
    ...
    if (instance) vue.onBeforeMount(fn, target);
    else if (sync) fn();
  ...
  function tryOnBeforeUnmount(fn, target) {
    const instance = getLifeCycleTarget(target);
    if (instance) vue.onBeforeUnmount(fn, target);
  }

  function tryOnMounted(fn, sync = true, target) {
    const instance = getLifeCycleTarget(target);
    if (instance) vue.onMounted(fn, target);
    else if (sync) fn();
    else vue.nextTick(fn);
  }
  ...
  function tryOnUnmounted(fn, target) {
    const instance = getLifeCycleTarget(target);
    if (instance) vue.onUnmounted(fn, target);
  }
        ...
        stop = vue.watch(
          ...
          (v) => {
            if (condition(v) !== isNot) {
              if (stop) stop();
              else vue.nextTick(() => (stop == null ? void 0 : stop()));
              resolve(v);
            }
          },
          {
  ...
  function useArrayDifference(...args) {
    ...
    const diff1 = vue.computed(() =>
      vue.toValue(list).filter((x) => vue.toValue(values).findIndex((y) => compareFn(x, y)) === -1)
    );
    if (symmetric) {
      const diff2 = vue.computed(() =>
        vue.toValue(values).filter((x) => vue.toValue(list).findIndex((y) => compareFn(x, y)) === -1)
      );
  ...
  function useArrayEvery(list, fn) {
    return vue.computed(() =>
      vue.toValue(list).every((element, index, array) => fn(vue.toValue(element), index, array))
    );
  }
  ...
  function useArrayFind(list, fn) {
    return vue.computed(() =>
      vue.toValue(vue.toValue(list).find((element, index, array) => fn(vue.toValue(element), index, array)))
    );
  }
  ...
  function useArrayFindIndex(list, fn) {
    return vue.computed(() =>
      vue.toValue(list).findIndex((element, index, array) => fn(vue.toValue(element), index, array))
    );
  }
    ...
    return vue.computed(() =>
      vue.toValue(
        !Array.prototype.findLast
          ? findLast(vue.toValue(list), (element, index, array) => fn(vue.toValue(element), index, array))
          : vue.toValue(list).findLast((element, index, array) => fn(vue.toValue(element), index, array))
      )
    );
  }
    ...
    return vue.computed(() =>
      vue
        ...
        .some((element, index, array) =>
          comparator(vue.toValue(element), vue.toValue(value), index, vue.toValue(array))
        )
    );
  }
  ...
  function useArrayReduce(list, reducer, ...args) {
    ...
    return vue.computed(() => {
      ...
      return args.length
        ? resolved.reduce(reduceCallback, typeof args[0] === 'function' ? vue.toValue(args[0]()) : vue.toValue(args[0]))
  ...
  function useArraySome(list, fn) {
    return vue.computed(() =>
      vue.toValue(list).some((element, index, array) => fn(vue.toValue(element), index, array))
    );
  }
  ...
  function useLastChanged(source, options = {}) {
    ...
    vue.watch(source, () => (ms.value = timestamp()), options);
  ...
  function watchArray(source, cb, options) {
    let oldList = (options == null ? void 0 : options.immediate)
      ? []
      : [...(typeof source === 'function' ? source() : Array.isArray(source) ? source : vue.toValue(source))];
    return vue.watch(
      source,
      (newList, _, onCleanup) => {
  ...
  function watchAtMost(source, cb, options) {
    ...
    const stop = watchWithFilter(
      ...
      (...args) => {
        current.value += 1;
        if (current.value >= vue.toValue(count)) vue.nextTick(() => stop());
        cb(...args);
      },
      watchOptions
    );
  ...
  function getWatchSources(sources) {
    if (vue.isReactive(sources)) return sources;
    if (Array.isArray(sources)) return sources.map((item) => vue.toValue(item));
  ...
  function whenever(source, cb, options) {
    const stop = vue.watch(
      ...
      (v, ov, onInvalidate) => {
        if (v) {
          if (options == null ? void 0 : options.once) vue.nextTick(() => stop());
