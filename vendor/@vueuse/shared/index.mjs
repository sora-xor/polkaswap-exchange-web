...
function reactiveOmit(obj, ...keys) {
  ...
  return reactiveComputed(() =>
    typeof predicate === 'function'
      ? Object.fromEntries(Object.entries(toRefs$1(obj)).filter(([k, v]) => !predicate(toValue$1(v), k)))
...
function useArrayDifference(...args) {
  ...
  const diff1 = computed(() =>
    toValue$1(list).filter((x) => toValue$1(values).findIndex((y) => compareFn(x, y)) === -1)
  );
  if (symmetric) {
    const diff2 = computed(() =>
      toValue$1(values).filter((x) => toValue$1(list).findIndex((y) => compareFn(x, y)) === -1)
    );
...
function useArrayFind(list, fn) {
  return computed(() =>
    toValue$1(toValue$1(list).find((element, index, array) => fn(toValue$1(element), index, array)))
  );
}
  ...
  return computed(() =>
    toValue$1(
      !Array.prototype.findLast
        ? findLast(toValue$1(list), (element, index, array) => fn(toValue$1(element), index, array))
        : toValue$1(list).findLast((element, index, array) => fn(toValue$1(element), index, array))
    )
  );
}
...
function useArrayIncludes(...args) {
  ...
  return computed(() =>
    toValue$1(list)
      .slice(formIndex)
      .some((element, index, array) => comparator(toValue$1(element), toValue$1(value), index, toValue$1(array)))
  );
}
...
function useArrayReduce(list, reducer, ...args) {
  ...
  return computed(() => {
    ...
    return args.length
      ? resolved.reduce(reduceCallback, typeof args[0] === 'function' ? toValue$1(args[0]()) : toValue$1(args[0]))
...
function watchArray(source, cb, options) {
  let oldList = (options == null ? void 0 : options.immediate)
    ? []
    : [...(typeof source === 'function' ? source() : Array.isArray(source) ? source : toValue$1(source))];
  return watch(
    source,
    (newList, _, onCleanup) => {
