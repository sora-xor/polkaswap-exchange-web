(function (exports, shared, vue) {
  ...
  function computedInject(key, options, defaultSource, treatDefaultAsFactory) {
    ...
    if (defaultSource) source = vue.inject(key, defaultSource);
    if (treatDefaultAsFactory) source = vue.inject(key, defaultSource, treatDefaultAsFactory);
    if (typeof options === 'function') {
      return vue.computed((oldValue) => options(source, oldValue));
    } else {
      return vue.computed({
  ...
  function createReusableTemplate(options = {}) {
    ...
    return shared.makeDestructurable({ define, reuse }, [define, reuse]);
  }
  function keysToCamelKebabCase(obj) {
    const newObj = {};
    for (const key in obj) newObj[shared.camelize(key)] = obj[key];
    return newObj;
  }
  ...
  function createTemplatePromise(options = {}) {
    ...
    const component = /*@__PURE__*/ vue.defineComponent((_, { slots }) => {
      ...
      if (options.transition) return () => vue.h(vue.TransitionGroup, options.transition, renderList);
      return renderList;
    });
  ...
  function onClickOutside(target, handler, options = {}) {
    ...
    if (shared.isIOS && !_iOSWorkaround) {
      ...
      Array.from(window.document.body.children).forEach((el) =>
        el.addEventListener('click', shared.noop, listenerOptions)
      );
  ...
  function onElementRemoval(target, callback, options = {}) {
    ...
    if (!window || !document) return shared.noop;
    let stopFn;
  ...
  function useAnimate(target, keyframes, options) {
    ...
    if (shared.isObject(options)) {
      config = options;
      animateOptions = shared.objectOmit(options, [
        'window',
        'immediate',
        'commitStyles',
        'persist',
    ...
    vue.watch(
      ...
      (value) => {
        if (animate.value) {
          ...
          if (targetEl) {
            animate.value.effect = new KeyframeEffect(targetEl, vue.toValue(value), animateOptions);
          }
        }
      },
      { deep: true }
    ...
    function update(init) {
      ...
      if (!animate.value) animate.value = el.animate(vue.toValue(keyframes), animateOptions);
  ...
  function useAsyncQueue(tasks, options) {
    const { interrupt = true, onError = shared.noop, onFinished = shared.noop, signal } = options || {};
    const promiseState = {
      aborted: 'aborted',
      fulfilled: 'fulfilled',
      pending: 'pending',
  ...
  function useAsyncState(promise, initialState, options) {
    ...
    async function execute(delay2 = 0, ...args) {
      if (resetOnExecute) state.value = vue.toValue(initialState);
      error.value = void 0;
      isReady.value = false;
      isLoading.value = true;
      if (delay2 > 0) await shared.promiseTimeout(delay2);
  ...
  function useBase64(target, options) {
    ...
    if (vue.isRef(target) || typeof target === 'function') vue.watch(target, execute, { immediate: true });
    else execute();
    return {
      base64,
      promise,
  ...
  function useBreakpoints(breakpoints, options = {}) {
    function getValue(k, delta) {
      ...
      if (delta != null) v = shared.increaseWithUnit(v, delta);
    ...
    function current() {
      const points = Object.keys(breakpoints)
        .map((k) => [k, shortcutMethods[k], shared.pxValue(getValue(k))])
        .sort((a, b) => a[2] - b[2]);
  ...
  function useStorage(key, defaults, storage, options = {}) {
    ...
    const { pause: pauseWatch, resume: resumeWatch } = shared.pausableWatch(data, (newValue) => write(newValue), {
      flush,
      deep,
      eventFilter,
    });
  ...
  function useColorMode(options = {}) {
    ...
    const store =
      storageRef ||
      (storageKey == null
        ? shared.toRef(initialValue)
        : useStorage(storageKey, initialValue, storage, { window, listenToStorageChanges }));
    ...
    const updateHTMLAttrs = getSSRHandler('updateHTMLAttrs', (selector2, attribute2, value) => {
      const el =
        typeof selector2 === 'string'
          ? window == null
            ? void 0
            : window.document.querySelector(selector2)
  ...
  function useCssVar(prop, target, options = {}) {
    ...
    const elRef = vue.computed(() => {
      var _a;
      return (
        unrefElement(target) || ((_a = window == null ? void 0 : window.document) == null ? void 0 : _a.documentElement)
      );
    });
  ...
  function useCycleList(list, options) {
    ...
    function getInitialValue() {
      var _a, _b;
      return (_b = vue.toValue(
        (_a = options == null ? void 0 : options.initialValue) != null ? _a : vue.toValue(list)[0]
      )) != null
        ? _b
        : void 0;
    }
  ...
  function useRefHistory(source, options = {}) {
    ...
    const { ignoreUpdates, ignorePrevAsyncUpdates, stop } = shared.watchIgnorable(source, commit, {
      deep,
      flush,
      eventFilter: composedFilter,
    });
  ...
  function useDeviceMotion(options = {}) {
    const { window = defaultWindow, requestPermissions = false, eventFilter = shared.bypassFilter } = options;
  ...
  function useDocumentVisibility(options = {}) {
    ...
    if (!document) return vue.shallowRef('visible');
  ...
  function useDraggable(target, options = {}) {
    ...
    const position = vue.ref((_a = vue.toValue(initialValue)) != null ? _a : { x: 0, y: 0 });
  ...
  function useElementByPoint(options) {
    ...
    const isSupported = useSupported(() => {
      if (vue.toValue(multiple)) return document && 'elementsFromPoint' in document;
      return document && 'elementFromPoint' in document;
    });
    const element = vue.shallowRef(null);
    const cb = () => {
      var _a, _b;
      element.value = vue.toValue(multiple)
        ? (_a = document == null ? void 0 : document.elementsFromPoint(vue.toValue(x), vue.toValue(y))) != null
          ? _a
          : []
        : (_b = document == null ? void 0 : document.elementFromPoint(vue.toValue(x), vue.toValue(y))) != null
          ? _b
          : null;
    };
    const controls =
      interval === 'requestAnimationFrame'
        ...
        : shared.useIntervalFn(cb, interval, { immediate });
    return {
      isSupported,
      element,
      ...controls,
  ...
  function useEventSource(url, events = [], options = {}) {
    ...
    if (immediate) open();
    if (autoConnect) vue.watch(urlRef, open);
  ...
  function useFetch(url, ...args) {
    ...
    if (timeout) timer = shared.useTimeoutFn(abort, timeout, { immediate: false });
    ...
    vue.watch([refetch, shared.toRef(url)], ([refetch2]) => refetch2 && execute(), { deep: true });
    const shell = {
      ...
      return (payload, payloadType) => {
        if (!isFetching.value) {
          ...
          if (vue.isRef(config.payload)) {
            vue.watch([refetch, shared.toRef(config.payload)], ([refetch2]) => refetch2 && execute(), { deep: true });
          }
          return {
            ...shell,
            then(onFulfilled, onRejected) {
  ...
  function useFileDialog(options = {}) {
    ...
    const inputRef = vue.computed(() => {
      var _a;
      const input =
        (_a = unrefElement(options.input)) != null ? _a : document ? document.createElement('input') : void 0;
      if (input) {
        input.type = 'file';
    ...
    const applyOptions = (options2) => {
      ...
      if (shared.hasOwn(options2, 'capture')) el.capture = vue.toValue(options2.capture);
    };
    const open = (localOptions) => {
      ...
      if (vue.toValue(mergedOptions.reset)) reset();
      el.click();
    };
    vue.watchEffect(() => {
  ...
  function useFullscreen(target, options = {}) {
    ...
    if (autoExit) shared.tryOnScopeDispose(exit);
    return {
      isSupported,
      isFullscreen,
      enter,
  ...
  function useImage(options, asyncStateOptions = {}) {
    const state = useAsyncState(() => loadImage(vue.toValue(options)), void 0, {
      resetOnExecute: true,
      ...asyncStateOptions,
    });
    vue.watch(
  ...
  function useInfiniteScroll(element, onLoadMore, options = {}) {
    ...
    const stop = vue.watch(() => [state.arrivedState[direction], isElementVisible.value], checkAndLoad, {
      immediate: true,
    });
  ...
  function usingElRef(source, cb) {
    if (vue.toValue(source)) cb(vue.toValue(source));
  }
  ...
  function useMediaControls(target, options = {}) {
    ...
    vue.watchEffect(() => {
      ...
      else if (shared.isObject(src)) sources = [src];
    ...
    useEventListener(target, 'durationchange', () => (duration.value = vue.toValue(target).duration), listenerOptions);
    useEventListener(
      target,
      'progress',
    ...
    useEventListener(target, 'ratechange', () => (rate.value = vue.toValue(target).playbackRate), listenerOptions);
  ...
  function useMemoize(resolver, options) {
    const initCache = () => {
      if (options == null ? void 0 : options.cache) return vue.shallowReactive(options.cache);
  ...
  function useMouseInElement(target, options = {}) {
    ...
    if (window) {
      ...
      const stopWatch = vue.watch([targetRef, x, y], update);
  ...
  function useNow(options = {}) {
    ...
    const controls =
      interval === 'requestAnimationFrame'
        ...
        : shared.useIntervalFn(update, interval, { immediate });
    if (exposeControls) {
      return {
        now,
        ...controls,
  ...
  function useOffsetPagination(options) {
    ...
    const pageCount = vue.computed(() => Math.max(1, Math.ceil(vue.toValue(total) / vue.toValue(currentPageSize))));
  ...
  function usePreferredLanguages(options = {}) {
    const { window = defaultWindow } = options;
    if (!window) return vue.shallowRef(['en']);
    const navigator = window.navigator;
    const value = vue.shallowRef(navigator.languages);
    useEventListener(
      window,
      'languagechange',
      () => {
  ...
  function useScriptTag(src, onLoaded = shared.noop, options = {}) {
    ...
    if (immediate && !manual) shared.tryOnMounted(load);
    if (!manual) shared.tryOnUnmounted(unload);
  ...
  function useScrollLock(element, initialState = false) {
    ...
    const unlock = () => {
      const el = resolveElement(vue.toValue(element));
      if (!el || !isLocked.value) return;
      if (shared.isIOS) stopTouchMoveListener == null ? void 0 : stopTouchMoveListener();
  ...
  function useSorted(...args) {
    ...
    if (!dirty) return vue.computed(() => sortFn([...vue.toValue(source)], compareFn));
    vue.watchEffect(() => {
      const result = sortFn(vue.toValue(source), compareFn);
      if (vue.isRef(source)) source.value = result;
  ...
  function useSpeechSynthesis(text, options = {}) {
    ...
    const bindEventsForUtterance = (utterance2) => {
      ...
      utterance2.volume = vue.toValue(volume);
      utterance2.onstart = () => {
  ...
  function useStyleTag(css, options = {}) {
    ...
    if (immediate && !manual) shared.tryOnMounted(load);
    if (!manual) shared.tryOnScopeDispose(unload);
    return {
      id,
      css: cssRef,
      unload,
  ...
  function useTextDirection(options = {}) {
    ...
    function getValue() {
      var _a, _b;
      return (_b =
        (_a = document == null ? void 0 : document.querySelector(selector)) == null
          ? void 0
  ...
  function useTimeAgoIntl(time, options = {}) {
    const { controls: exposeControls = false, updateInterval = 3e4 } = options;
    const { now, ...controls } = useNow({ interval: updateInterval, controls: true });
    const result = vue.computed(() => getTimeAgoIntlResult(new Date(vue.toValue(time)), options, vue.toValue(now)));
    const parts = vue.computed(() => result.value.parts);
  ...
  function useTimeoutPoll(fn, interval, options = {}) {
    ...
    if (immediate && shared.isClient) resume();
    shared.tryOnScopeDispose(pause);
    return {
      isActive,
      pause,
  ...
  function useTitle(newTitle = null, options = {}) {
    ...
    const title = shared.toRef(
      (_b = newTitle != null ? newTitle : document == null ? void 0 : document.title) != null ? _b : null
    );
  ...
  function executeTransition(source, from, to, options = {}) {
    ...
    const trans =
      typeof options.transition === 'function'
        ? options.transition
        : (_b = vue.toValue(options.transition)) != null
          ? _b
          : shared.identity;
  ...
  function useTransition(source, options = {}) {
    ...
    vue.watch(
      ...
      async (to) => {
        ...
        const id = ++currentId;
        if (options.delay) await shared.promiseTimeout(vue.toValue(options.delay));
  ...
  function useUrlSearchParams(mode = 'history', options = {}) {
    ...
    if (!window) return vue.reactive(initialValue);
    const state = vue.reactive({});
  ...
  function useWatchForSizes(size, list, containerRef, calculateRange) {
    vue.watch([size.width, size.height, () => vue.toValue(list), containerRef], () => {
      calculateRange();
    });
  }
  ...
  function useWebNotification(options = {}) {
    ...
    const permissionGranted = vue.shallowRef(
    ...
    if (_requestForPermissions) shared.tryOnMounted(ensurePermissions);
    shared.tryOnScopeDispose(close);
    if (isSupported.value && window) {
      const document = window.document;
  ...
  function useWebSocket(url, options = {}) {
    ...
    const close = (code = 1e3, reason) => {
      resetRetry();
      if ((!shared.isClient && !shared.isWorker) || !wsRef.value) return;
      explicitlyClosed = true;
      resetHeartbeat();
    ...
    const _init = () => {
      ...
      ws.onmessage = (e) => {
        if (options.heartbeat) {
          ...
          if (e.data === vue.toValue(responseMessage)) return;
        }
        data.value = e.data;
    ...
    if (autoClose) {
      if (shared.isClient) useEventListener('beforeunload', () => close(), { passive: true });
      shared.tryOnScopeDispose(close);
    }
    const open = () => {
      if (!shared.isClient && !shared.isWorker) return;
      close();
      explicitlyClosed = false;
      retried = 0;
      _init();
  ...
  function useWindowFocus(options = {}) {
    const { window = defaultWindow } = options;
    if (!window) return vue.shallowRef(false);
