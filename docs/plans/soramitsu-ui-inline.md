...
4. **Run the verification matrix.** Execute `yarn build`, `yarn build:vue3`, `yarn test:translation`, and `yarn test:unit`. If Playwright wallet smokes are tagged, run `VITE_DISABLE_COMPAT=true yarn test:e2e --project chromium --grep @wallet` (or the scoped tag documented in the roadmap). Record pass/fail status in the pull request description.
