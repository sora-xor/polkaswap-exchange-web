...
- Detect residual legacy-runtime regressions or translation gaps after the Vue 3 rollout.
...
| `build_variant_selected` | Track the shipped runtime variant        | During app bootstrap (`src/main.ts`)                                                                | `variant`, `environment`, `timestamp`                              |
...
2. **Build variant instrumentation**
   - In `src/main.ts`, expose the shipped `buildVariant` (`vue3-native`) during bootstrap.
...
6. **Analytics routing**
   - Map telemetry events to Amplitude/DataDog dashboards:
     ...
     - `translation_missing` → Error triage board.
